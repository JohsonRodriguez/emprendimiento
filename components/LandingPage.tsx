...
export function LandingPage({locale}:{locale:Locale}){const d=dictionaries[locale]; return <>
  <Navbar locale={locale} d={d}/><main id="top">
    ...
    <section className="section results-section"><Reveal><h2>{d.resultsTitle}</h2></Reveal><div className="results-orbit"><div className="result-core"><span>Tueny</span><Image className="result-brand-mark" src={assetPath("/brand/tueny-symbol-white.svg")} alt="Tueny symbol" width={54} height={54}/></div>{d.results.map((x,i)=><Reveal className={`result-chip chip-${i}`} delay={i*.04} key={x}><Check size={17}/>{x}</Reveal>)}</div></section>
