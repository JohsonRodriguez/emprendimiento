...
export function LandingPage({locale}:{locale:Locale}){const d=dictionaries[locale]; return <>
  <Navbar locale={locale} d={d}/><main id="top">
    <section className="hero"><div className="hero-copy"><Reveal><p className="eyebrow">{d.heroKicker}</p><h1>{d.heroTitle}</h1><p className="hero-body">{d.heroBody}</p><div className="hero-actions"><a className="button primary" href="#contact">{d.heroPrimary}<ArrowRight size={18}/></a><a className="text-link" href="#before-after">{d.heroSecondary}<ArrowDownRight size={18}/></a></div></Reveal></div><Reveal className="hero-visual" delay={.12}><div className="hero-image"><Image src="/images/hero-transformation.png" alt="Laptop comparing an outdated website with its modern redesign" fill priority sizes="(max-width: 900px) 100vw, 56vw"/></div><div className="transform-caption"><span>{d.before}</span><i/><strong>{d.after}</strong></div></Reveal></section>
    <section className="trust-strip" aria-label="Website standards">{d.trust.map((x)=><span key={x}><Check size={17}/>{x}</span>)}</section>

    <section className="section problem-section"><Reveal className="section-heading"><h2>{d.problemTitle}</h2><p>{d.problemBody}</p></Reveal><div className="problem-grid">{d.problems.map((x,i)=><Reveal className={`problem-item p${i}`} delay={i*.04} key={x}><span>{String(i+1).padStart(2,"0")}</span><p>{x}</p></Reveal>)}</div></section>
  ...
  </main><footer><div><Link href={`/${locale}`} className="logo footer-logo"><span>N</span>Nueva Web</Link><p>{d.footer}</p></div><nav>{d.nav.slice(1).map((x,i)=><a key={x} href={`#${["services","process","before-after","work","about","contact"][i]}`}>{x}</a>)}</nav><div className="footer-meta"><span>© {new Date().getFullYear()} Nueva Web. {d.rights}</span><span><a href="#">{d.privacy}</a><a href="#">{d.terms}</a><Link href={locale==="es"?"/en":"/es"}>{locale==="es"?"English":"Español"}</Link></span></div></footer>
  </>}
