...
import { ArrowDownRight, ArrowRight, Check, DeviceMobile, Gauge, GlobeHemisphereWest, MagnifyingGlass, PaintBrush, ShieldCheck, Wrench } from "@phosphor-icons/react/dist/ssr";
...
export function LandingPage({locale}:{locale:Locale}){const d=dictionaries[locale]; return <>
  <Navbar locale={locale} d={d}/><main id="top">
    <section className="hero"><div className="hero-copy"><Reveal><p className="eyebrow">{d.heroKicker}</p><h1>{d.heroTitle}</h1><p className="hero-body">{d.heroBody}</p><div className="hero-actions"><a className="button primary" href="#contact">{d.heroPrimary}<ArrowRight size={18}/></a></div></Reveal></div><Reveal className="hero-visual" delay={.12}><div className="hero-image"><Image src={assetPath("/images/hero-transformation.webp")} alt="Laptop comparing an outdated website with its modern redesign" fill priority sizes="(max-width: 900px) 100vw, 56vw"/></div><div className="transform-caption"><span>{d.before}</span><i/><strong>{d.after}</strong></div></Reveal></section>
    ...
    <section className="section work-section" id="work"><Reveal className="section-heading compact"><p className="eyebrow">{locale==="es"?"Proyectos seleccionados":"Selected work"}</p><h2>{d.portfolioTitle}</h2></Reveal><div className="projects">{d.projects.map((project,i)=><Reveal className={`project project-${i}`} key={project[0]}><div className="project-image"><Image src={assetPath(i?"/images/project-restaurant.webp":"/images/project-dental.webp")} alt={`${project[0]} website redesign`} fill sizes="(max-width:768px) 100vw, 70vw"/></div><div className="project-info"><h3>{project[0]}</h3><dl><div><dt>{d.problem}</dt><dd>{project[1]}</dd></div><div><dt>{d.solution}</dt><dd>{project[2]}</dd></div><div><dt>{d.result}</dt><dd>{project[3]}</dd></div></dl></div></Reveal>)}</div></section>

    <section className="section results-section"><Reveal><h2>{d.resultsTitle}</h2></Reveal><div className="results-orbit"><div className="result-core"><div className="result-brand-lockup"><Image className="result-brand-mark" src={assetPath("/brand/tueny-symbol-white.svg")} alt="" width={44} height={44}/><span>Tueny</span></div></div>{d.results.map((x,i)=><Reveal className={`result-chip chip-${i}`} delay={i*.04} key={x}><Check size={17}/>{x}</Reveal>)}</div></section>
  ...
  </main><footer><div><Link href={`/${locale}`} className="logo footer-logo"><Image className="brand-mark" src={assetPath("/brand/tueny-symbol-white.svg")} alt="" width={34} height={34}/>Tueny</Link><p>{d.footer}</p></div><nav>{d.nav.slice(1).map((x,i)=><a key={x} href={`#${["services","process","before-after","work","contact"][i]}`}>{x}</a>)}</nav><div className="footer-meta"><span>© {new Date().getFullYear()} Tueny. {d.rights}</span><span><a href="#">{d.privacy}</a><a href="#">{d.terms}</a><Link href={locale==="es"?"/en":"/es"}>{locale==="es"?"English":"Español"}</Link></span></div></footer>
  </>}
