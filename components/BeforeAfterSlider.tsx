...
export function BeforeAfterSlider({ before, after, drag }: { before:string; after:string; drag:string }) {
  ...
  return <div className="compare-wrap"><div className="compare" style={{"--pos":`${position}%`} as React.CSSProperties}>
    <Image src={assetPath("/images/hero-transformation.png")} alt={`${before}: outdated website shown on a laptop`} fill sizes="(max-width: 768px) 100vw, 1100px" className="compare-image old"/>
    <div className="new-clip"><Image src={assetPath("/images/hero-transformation.png")} alt={`${after}: redesigned modern website shown on a laptop`} fill sizes="(max-width: 768px) 100vw, 1100px" className="compare-image modern"/></div>
