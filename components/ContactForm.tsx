...
export function ContactForm({ d }: { d: Dictionary }) {
  ...
  return <form className="contact-form" onSubmit={submit} noValidate>{fields.map((f,i)=><label key={f.name}><span>{d.labels[i]}</span><input name={f.name} type={f.type} required={f.req} aria-invalid={errors.includes(f.name)}/>{errors.includes(f.name)&&<small>{d.required}</small>}</label>)}
    <label><span>{d.labels[5]}</span><select name="type" defaultValue=""> <option value="" disabled>Select</option>{d.types.map(x=><option key={x}>{x}</option>)}</select></label><label><span>{d.labels[6]}</span><select name="budget" defaultValue=""><option value="" disabled>Select</option><option>$1,500 - $3,000</option><option>$3,000 - $6,000</option><option>$6,000+</option><option>Not sure yet</option></select></label><label className="full"><span>{d.labels[7]}</span><textarea name="message" rows={5} required aria-invalid={errors.includes("message")}/>{errors.includes("message")&&<small>{d.required}</small>}</label><label className="checkbox full"><input type="checkbox" name="audit"/><span>{d.analyze}</span></label>
